...
class SYHelloWorldPass : public llvm::PassInfoMixin<SYHelloWorldPass> {
   public:
...
JITCompiler::JITCompiler() : builder_(context_) {
    module_ = new llvm::Module("JIT! COOOL!", context_);
    named_values_.clear();

    module_pass_manager_     = new llvm::ModulePassManager();
    module_analysis_manager_ = new llvm::ModuleAnalysisManager();
}
...
llvm::Value* JITCompiler::compilerError(std::string msg, int line) {
    fprintf(
        stderr,
        "\033[1m\033[31mError in compiling\033[0m: line \033[1m%d\033[0m: %s\n",
        line, msg.c_str());
    return nullptr;
}
        ...
        switch (exp->a_->a_->getAstType()) {
            ...
            case SyAstType::LOGIC_NOT:
                ...
                ret = builder_.CreateICmpNE(
                    ...
                    llvm::ConstantInt::get(context_, llvm::APInt(32, 0, true)),
                    "tmp");
                ret = builder_.CreateXor(
                    ret,
                    llvm::ConstantInt::get(context_, llvm::APInt(1, 1, true)),
                    "nottmp");
                return builder_.CreateZExt(
                    ret, llvm::Type::getInt32Ty(context_), "exttmp");
            default:
                break;
        }
        return nullptr;
...
llvm::Value* JITCompiler::lValRightIRGen(AstNodePtr l_val) {
    ...
    if (l_val_ir == nullptr) {
        ...
        return builder_.CreateLoad(llvm::Type::getInt32Ty(context_), l_val_ir,
                                   "loadtmp");
    }
}
...
llvm::Value* JITCompiler::constExpIRGen(AstNodePtr const_exp) {
    // to const exp, we know that const_exp->u_.const_val_ is valid, so just use
    // that. just in case that the jit is called before interpret, we do a check
    // on const_exp->u_.const_val_
    ...
    return llvm::ConstantInt::get(
        context_, llvm::APInt(32, const_exp->u_.const_val_, true));
}
...
llvm::Value* JITCompiler::subExpIRGen(AstNodePtr exp) {
    ...
    switch (exp->b_->getAstType()) {
        ...
        case SyAstType::ALU_MOD:
            ...
            // are: eq: equal ne: not equal ugt: unsigned greater than uge:
            // unsigned greater or equal ult: unsigned less than ule: unsigned
            // less or equal sgt: signed greater than sge: signed greater or
            // equal slt: signed less than sle: signed less or equal
        case SyAstType::EQ:
            cmp_ir = builder_.CreateICmpEQ(ir_a, ir_c, "eqtmp");
            ret_ir = builder_.CreateIntCast(
                cmp_ir, llvm::Type::getInt32Ty(context_), true, "boolToInttmp");
            break;
        case SyAstType::NEQ:
        ...
        case SyAstType::LOGIC_AND:
            ...
            ret_ir = builder_.CreateIntCast(
                ret_ir, llvm::Type::getInt32Ty(context_), true, "boolToInttmp");
            break;
...
// we assert that stmt->a_->getAstType() == SyAstType::STM_IF
    ...
    cond_ir = builder_.CreateICmpEQ(
        cond_ir, llvm::ConstantInt::get(llvm::Type::getInt32Ty(context_), 1),
        "ifcond");
    ...
    // set the then_basic_block as the current insert point is mainly for the
    // PHI of this
    // if-else return val in the document, as our if-else return nothing, we
    // don't need to
    // thus the following two lines of code won't be included
    // Codegen of 'Then' can change the current block, update then_basic_block
    // for the PHI.
...
llvm::Value* JITCompiler::stmtIRGen(AstNodePtr stmt) {
    ...
    switch (stmt->a_->getAstType()) {
        ...
        case SyAstType::STM_RETURN:
            // TODO: maybe a return type checking? though this shoudn't be a
            // problem here
            if (stmt->b_ != nullptr) {
...
llvm::Value* JITCompiler::blockIRGen(AstNodePtr block) {
    ...
    for (auto block_item = block->a_; block_item != nullptr;
         block_item      = block_item->d_) {
        ret = blockItemIRGen(block_item);
    }
    return ret;
}

llvm::AllocaInst* JITCompiler::createFunctionArgReAlloca(
    llvm::Function* this_function, const llvm::Argument& arg) {
    llvm::IRBuilder<> tmp_builder(&this_function->getEntryBlock(),
                                  this_function->getEntryBlock().begin());
    llvm::AllocaInst* alloca_inst =
        tmp_builder.CreateAlloca(arg.getType(), nullptr, arg.getName());
    return alloca_inst;
}
...
llvm::Function* JITCompiler::functionDefinitionLLVMIrGen(AstNodePtr func) {
    ...
    llvm::BasicBlock* entry_block =
        llvm::BasicBlock::Create(context_, "entry", this_func);
    ...
    /*
    function_pass_manager.addPass(SYHelloWorldPass());
    module_pass_manager_->addPass(llvm::createModuleToFunctionPassAdaptor(
        std::move(function_pass_manager)));
    module_pass_manager_->run(*module_, *module_analysis_manager_);
    */
    ...
    switch (func_type_enum) {
        case SyAstType::TYPE_INT:
            func_type = llvm::FunctionType::get(
                llvm::Type::getInt32Ty(context_), argv, false);
            break;
        case SyAstType::TYPE_VOID:
            func_type = llvm::FunctionType::get(llvm::Type::getVoidTy(context_),
                                                argv, false);
            break;
        default:
